...
```text
จากโจทย์จะมีปราสาทรูปแบบพีรามิด ซึ่งพีระมิดทีหลายชั้นและชั้นที่อยู่ข้างบนสุดคือชั้นหนึ่ง การจะขึ้นไปชั้นบนจำเป็นจะต้องพังกำแพงขึ้นไป โจทย์ต้องการให้รับค่าว่าเราอยู่ห้องหมายเลขอะไรและจะเดินทางไปห้อง 1 ให้พังกำแพงน้อยสุดกี่กำแพง

Input:
หมายเลขห้องที่อยู่ปัจจุบัน

Output:
จำนวนกำแพงที่ต้องพังขึ้นไป

Contraints:
จากการดูแพทเทิร์นของโจทย์ หากห้องที่เราอยู่เป็นสามเหลี่ยมชี้ลงจะสามารถพังขึ้นได้เลยแต่ถ้าเป็นสามเหลี่ยมชี้ขึ้นต้องพังไปยังอีกห้องก่อนถึงจะขึ้นชั้นถัดไปได้
```
...
```text
step 1: รับค่าคิดรากที่สองแล้วปัดขึ้น (หาว่าเราอยู่ชั้นที่เท่าไหร่)
step 2: สร้างเงื่อนไขที่ว่าหากอยู่ชั้นคู่และห้องเป็นเลขคี่ (สามเหลี่ยมชี้ลง) ให้คำนวณโดยการนำจำนวนชั้นที่ได้ลบ 1 ออกแล้วคูณ 2 จากนั้นลบด้วย 1 อีกที (ลบชั้นแรกออก , คูณสองเพราะการพังกำแพงแต่ละชั้นพังทีละ 2 , ลบออกด้วย 1 เนื่องจากสามารถขึ้นชั้นถัดไปได้เลยไม่ต้องพังกำแพงข้าง ๆ)
step 3: สร้างเงื่อนไขหากอยู่ชั้นคี่แต่ห้องเป็นเลขคู่ ให้คำนวณเหมือน step2
step 4: หากไม่เข้าเงื่อนไขใดเลยให้คำนวณโดยการนำจำนวนชั้นมาลบ 1 คูณด้วย 2
step 5: พิมพ์จำนวนกำแพงที่ต้องพังออกมา

ข้อนี้ได้รับความช่วยเหลือจากรุ่นพี่ในการหาแพทเทิร์นจากโจทย์ และได้คู่ pair ช่วยไกด์ไลน์โค้ดให้ว่าควรสร้างเงื่อนไขอย่างไร
```
...
```text
วิธีสุดท้ายเหมือนแผนแรก คือหาว่าเราอยู่ชั้นไหน และห้องที่อยู่เป็นเลขคู่หรือคี่ เพื่อที่จะได้จำแนกว่าเป็นสามเหลี่ยมขึ้นหรือลง จากนั้นคำนวณแยกตามกรณีและพิมพ์จำนวนกำแพงที่ต้องพังออกมา
```
...
```text
กรณีที่ชั้นเป็นเลขคู่แต่ห้องเป็นเลขคี่ ตรวจว่าสามารถรันโค้ดได้อย่างถูกต้องไหม
```
...
```text
กรณีที่ชั้นเป็นเลขคี่แต่ห้องเป็นเลขคู่ ตรวจว่าสามารถรันโค้ดได้อย่างถูกต้องไหม
```
...
```text
กรณีที่ห้องที่เราอยู่เป็นสามเหลี่ยมชี้ขึ้น ตรวจว่าสามารถรันโค้ดได้อย่างถูกต้องไหม
```
...
```text
คู่ pair และรุ่นพี่ในคณะ
```

เขาช่วยอะไร

```text
ช่วยให้มองแพทเทิร์นออกและคิดเงื่อนไขผ่าน
```

คุณยังทำอะไรด้วยตนเอง

```text
คิด testcase และหา testcase ที่รันไม่ผ่าน
```
