...
```text
โจทย์ให้รับตัวย่อของไพ่แต่ละใบแล้วแปลงเป็นชื่อเต็ม ไพ่ที่มีแต้ม 2-10 เรียกตามค่าของไพ่ หากเป็นเลข A คือ ace , J คือ jack , Q คือ queen , K คือ king และอีกส่วนคือสัญลักษณ์ D คือ diamonds ,H คือ หัวใจ ,S คือ spades ,C คือ clubs เรียกชื่อไพ่โดย แต้ม of สัญลักษณ์
Input : รับแต้มและกลุ่มของไพ่
Output : ชื่อของไพ่
Constraints : ระวังพิมพ์ชื่อของไพ่ผิด เนื่องจากมีชื่อหลากหลายตัวอักษร
```
...
```text
Step 1 : รับ input เข้ามาแบบตัดช่องว่างออก
Step 2 : ทำให้ตัวท้าย และตัวที่เหลือข้างหน้าเป็นตัวใหญ่
Step 3 : แปลงแต้มไพ่โดยสร้างเงื่อนไขจากโจทย์
โดย  A - ace , J - jack , Q - queen , K - king หากไม่ตรงกับตัวอักษรให้เก็บค่าเป็นตัวเลขนั้น ๆ
Step 4 : แปลงสัญลักษณ์
โดย D - diamonds , H - hearts , S - spades , C - clubs
Step 5 : แสดงผลในรูป {แต้ม} of {สัญลักษณ์}
```
...
```text
วิธีสุดท้ายของฉันเหมือนวิธีแรก โดยเพิ่มเติมเล็กน้อยคือมีการตั้งค่าเริ่มต้นเพื่อไม่ให้ติด pep8 รับค่ามา แยกไพ่และแปลงแต้มและสัญลักษณ์ก่อนจะพิมพ์ออกมา
```
...
```text
กรณีพิมพ์เล็ก : ตรวจสอบการทำงานของ upper
```
...
```text
เลข 2 หลัก : ตรวจสอบการทำงานของการแยกหลัก
```
...
```text
เลข 1 หลัก : ตรวจสอบการทำงานตัวเลขผสมตัวเล็ก
```
